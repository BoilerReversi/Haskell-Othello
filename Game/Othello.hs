...
outputList :: Bool -> Bool -> Board -> [OutputSquare]
outputList False False (Board grid _) = elems $ fmap toOutputSquare grid
outputList True False b@(Board grid _) = elems $ foldr (\x acc -> acc // [(x, L)]) os $ legalMoves b
  where
